...
const QnaForm = ({ pageIndex, setPageIndex, onNext }: QnaFormProps) => {
    ...
    const isPageAnswered = currentPage.qnaList.every(
        (qna) => answers[qna.qnaId]
    );

  return (
        <T.ListOnboardingContainer style={{ gap: "0rem"}}>
            <T.MbtiP style={{ margin: "1rem 0 3.4rem 0" }}>{currentPage.title}</T.MbtiP>
            ...
            <S.BottomContainer style={{ marginTop: "9.8rem" }}>
                <S.NextButton onClick={handleNext} disabled={!isPageAnswered} active={isPageAnswered}>다음</S.NextButton>
            </S.BottomContainer>
